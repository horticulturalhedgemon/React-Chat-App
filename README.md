...
a.
Equitable use is followed: The application makes no distinction between any kind of user. All users are treated the same.
Simple and intuitive use is followed: There is confirmation after taking each action: logging in, posting, registering, logging out, deleting a post. Buttons and directions are clearly labeled, and each page had a large title at the top.
b.
Flexibility in use is not followed: There is only one way to do anything. There is no customizability on the website.
Tolerance for error is not followed: There are no warnings or fail-safes for irreversible actions such as deleting a post.
...
![](_figures/IMG_20231023_214911_531.jpg)
### Done! 🥳
